...
def convert_ttl(input_path: str) -> None:
    # Load Turtle file
    g = Graph()
    ...
    # OWL/XML
    owl_path = base_name + ".owl"
    g.serialize(destination=owl_path, format='xml')
    print(f"Saved OWL/XML to {owl_path}")
    ...
    # Turtle (keep original but can regenerate if needed)
    ttl_path = base_name + ".ttl"
    if ttl_path != input_path:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python convert.py your_ontology.ttl")
        sys.exit(1)

    convert_ttl(sys.argv[1])
